<div class="col-md-7">
    <div class="panel panel-default">
        ...
        <h2 class="panel-title">Procurement Chain Stages Informations</h2>
    </div>
        <?php
            global $DBInstance;
            $page = TenderStage::getPage($DBInstance->escapeValues(trim($_GET['pg'])));
            $per_page = TenderStage::getRecordPerPage();
            $total_count = TenderStage::getCountAll();
        ?>
        <?php  $pagination = new Pagination($page, $per_page, $total_count); ?>
        <?php
            $result = TenderStage::findRecordsForThisPage($per_page, $pagination->getOffset());
            $tenderstages = TenderStage::getBySQL($result);
        ?>
                    ...
                    <div class="well">
                        <form action="actionid.php?pg=1" method="post">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="tender_stage_name" name="tender_stage_name" placeholder="ID Card Type Name">
                            </div>
                            <div class="form-group">
                                <label for="tender_stage_desc" class="sr-only">ID Card Type Description</label>
                                <textarea class="form-control" id="tender_stage_desc" name="tender_stage_desc" rows="3" placeholder="ID Card Type Description..."></textarea>
                            </div>
                            <div class="form-group">
                                <input type="hidden" id="tenderstage_id" name="tenderstage_id">
                            </div>
                            <div class="clearfix">
                                <button class="btn btn-primary pull-right" id="idcard_update_btn" type="submit" name="add_stage_name">Update</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <?php $counter = 1; ?>
                    ...
                    <?php global $DBInstance; ?>
                        <tr>
                            ...
                            <td class="hidden"><?php echo $DBInstance->HTMLEntitiesDecoding($tenderstage->id); ?></td>
                            <td class=""><?php echo $DBInstance->HTMLEntitiesDecoding($tenderstage->title); ?></td>
                            <td class=""><?php echo $DBInstance->HTMLEntitiesDecoding($tenderstage->description); ?></td>
                            <td class="">
                            ...
                            <td class=""><a href="actiontendpro.php?id=<?php echo $DBInstance->HTMLEntitiesDecoding($tenderstage->id); ?>"><button type="button" class="btn btn-danger btn-xs">Delete Info</button></a></td>
                        </tr>
