<?php
    ...
    class RequirementDocument extends DatabaseMANI {

        protected static $table_name = "requirement_forms";
        protected static $db_fields = array('id', 'title', 'body', 'filename', 'type', 'size', 'posteruid', 'timestamp');
        ...
        private $temp_path;
        protected $upload_directory = "requirement_forms";
        public $errors = array();
        ...
        public function attachDocumentPropsToInstanceVars($file) {
            global $session;

            if(!$file || empty($file) || !is_array($file)) {
                $this->errors[] = "No file was uploaded.";
                $session->message("No file was uploaded.");
                redirect_to("../tenderform/index.php?pg=1index.php?pg=1");
                return false;
        ...
        public function addDocumentInstanceVar($documentID, $title, $body, $posteruid, $timestamp) {
            if(!empty($documentID) && !empty($title) && !empty($body) && !empty($posteruid) && !empty($timestamp)) {
                $this->id = $documentID;
                $this->title = $title;
                $this->body = $body;
                ...
                return true;
            }else if(empty($documentID) && !empty($title) && !empty($body) && !empty($posteruid) && !empty($timestamp)){
                $this->title = $title;
                $this->body = $body;
                $this->posteruid = $posteruid;
                $this->timestamp = $timestamp;
        ...
        public function preparingDataForSubmission() {
            ...
            if(file_exists($target_path)) {
                if(RequirementDocument::isDataAssociatedWithExistInDB('filename', $this->filename)) {
                ...
                if(RequirementDocument::isDataAssociatedWithExistInDB('filename', $this->filename)) {
                    if (RequirementDocument::customDelete('filename', $this->filename)) {
                        $this->saveDataToDatabase();
                    }else {
        ...
        public function saveDataToDatabase() {
            ...
            if(isset($this->id)) {
                $this->update();
                unset($_SESSION['document_title']);
                unset($_SESSION['document_desc']);
                unset($_SESSION['document_id']);
                $session->message("Requirement Document $this->filename is successfully Updated");
                return true;
                ...
                if(move_uploaded_file($this->temp_path, $target_path)) {
                    if($this->create()){
                        ...
                        unset($_SESSION['document_title']);
                        unset($_SESSION['document_desc']);
                        $session->message("Requirement Document <b>$this->filename</b> is successfully submitted");
                        return true;
                    }else {
                        unset($this->id);
                        unset($this->title);
                        ...
                        unset($_SESSION['document_title']);
                        unset($_SESSION['document_desc']);
                        $session->message("Failed to submit <b>$this->filename</b> Requirement Document");
                    }
                }else {
        ...
        public function deleteDocuments() {
            ...
            if($this->delete()) {
                ...
                $session->message("Failed to delete <strong>".$this->title."<strong> Requirement Document!");
                return false;
                redirect_to("../tenderform/index.php?pg=1");
            }
        }
